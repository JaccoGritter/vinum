...
<div class="pl-3">
            ...
            <tr><td>{{$wine->brand}}</td><td>{{$wine->name}}</td><td>€{{ number_format($wine->pivot->myprice, 2) }}</td><td>{{$wine->pivot->quantity}}</td><td><a href="{{route('addone', $wine->pivot->id)}}"><i class="fas fa-plus fa-xs add"></i></a></td><td><a href="{{route('decreaseone', $wine->pivot->id)}}"><i class="fas fa-minus fa-xs add"></i></a></td></tr>
            @endforeach
